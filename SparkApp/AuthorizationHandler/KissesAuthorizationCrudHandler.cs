...
{
    public class KissesAuthorizationCrudHandler : AuthorizationHandler<OperationAuthorizationRequirement, SparkService.Models.Kisses>
    {
        private readonly SubscriptionService _subscriptionService;
        private readonly string serviceName = "kisses";

        public KissesAuthorizationCrudHandler(SubscriptionService subscriptionService) => (_subscriptionService) = (subscriptionService);
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, OperationAuthorizationRequirement requirement, SparkService.Models.Kisses resource)
        {
        ...
        private List<OperationAuthorizationRequirement> GetAllowedOperations(ClaimsPrincipal user, SparkService.Models.Kisses resource)
        {
            ...
            else if (user.IsInRole("User"))
            {
                ...
                else if (resource.kissed_id == user.Claims.FirstOrDefault(x => x.Type == "id")!.Value)
                {
