...
{
    public class TraitAuthorizationCrudHandler : AuthorizationHandler<OperationAuthorizationRequirement, SparkService.Models.UserTraits>
    {
        private readonly SubscriptionService _subscriptionService;
        private readonly string serviceName = "traits";

        public TraitAuthorizationCrudHandler(SubscriptionService subscriptionService) => (_subscriptionService) = (subscriptionService);
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, OperationAuthorizationRequirement requirement, SparkService.Models.UserTraits resource)
        {
        ...
        private List<OperationAuthorizationRequirement> GetAllowedOperations(ClaimsPrincipal user, SparkService.Models.UserTraits resource)
        {
        ...
        private bool DoesSubscriptionAllowForThisService(ClaimsPrincipal user)
        {
            ...
            try
            {
                ...
                Subscriptions? subscriptions = _subscriptionService.GetSubscriptions().FirstOrDefault(x => x.userId == userId && x.status.ToLower() == SubscriptionsStatus.active.ToString().ToLower());
